...
class LuaTypeRecoveryPassGenerator(cpg: Cpg, config: XTypeRecoveryConfig = XTypeRecoveryConfig(iterations = 3))
  ...
  override protected def generateRecoveryPass(
    ...
    new LuaTypeRecovery(cpg, state, iteration)
}

private class LuaTypeRecovery(cpg: Cpg, state: XTypeRecoveryState, iteration: Int)
  ...
  override def generateRecoveryForCompilationUnitTask(
    ...
    new RecoverForLuaFile(cpg, unit, builder, state)
  }
}

private class RecoverForLuaFile(cpg: Cpg, cu: NamespaceBlock, builder: DiffGraphBuilder, state: XTypeRecoveryState)
