...
import io.joern.lua2cpg.passes.{
  AstCreationPass,
  AnyTypePass,
  LocalCreationPass,
  LuaSetKnownTypesPass,
  LuaTypeRecoveryPassGenerator}
...
object Lua2Cpg {
  ...
  def postProcessingPasses(cpg: Cpg, config: Option[Config] = None): List[CpgPassBase] = {
    ...
    List(new LuaSetKnownTypesPass(cpg)) ++ new LuaTypeRecoveryPassGenerator(cpg, typeRecoveryConfig).generate()
  }
}